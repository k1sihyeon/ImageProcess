...
int main(int argc, char *argv[]) {
  ...
  int *num = malloc(sizeof(int) * cpnum);
  ...
  for (int i = 0; i < cpnum; i++) {
    ...
    if (pids[tidx] == -1) {
      ...
      //lseek(ifd1, start * n * sizeof(double), SEEK_SET);
      double *mat_A = (double *) Mmap(NULL, sizeof(double) * n * matSize, PROT_READ, MAP_SHARED, ifd1, sizeof(double) * n * start); 
      ...
      if(n == 1) {
        ...
        //printf("\n\ntidx:%d, start: %d, end: %d, matSize: %d \n", tidx, start, end, matSize);
        ...
        for (int row = 0; row < matSize; row++) {
          for (int col = 0; col < n; col++) {
            for (int k = 0; k < n; k++) {
              mat_C[row * n + col] += mat_A[row * n + k] * mat_B[k * n + col];
            }

            //printf("tidx: %d, matC[%d][%d]\n", tidx, row, col);
          }
        }
      ...
      close(ifd1);
      munmap(mat_A, n * matSize * sizeof(double));
