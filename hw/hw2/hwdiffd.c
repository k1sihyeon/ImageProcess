...
/* 두 파일이 n*n개의 double float 실수 (행렬)들로 이루어져 있고, 
두 파일의 각 실수들의 차이가 세 번째 argument보다 크면 error를 출력한다. 
두 파일의 크기는 n*n*sizeof(double) byte이어야 한다. 
아래 예서는 c1.dat 파일과 c2.dat 파일을 비교할 때 각 float element의 차이가 0.001 미만일 때는 동일한 것으로 판정하여 0을 returns하고 
그렇지 않으면 1을 return한다.
