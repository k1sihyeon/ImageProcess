...
/* 두 파일이 n*n개의 double float 실수 (행렬)들로 이루어져 있고, 두 파일의 각 실수들의 차이가 세 번째 argument보다 크면 error를 출력한다. 두 파일의 크기는 n*n*sizeof(double) byte이어야 한다. 아래 예서는 c1.dat 파일과 c2.dat 파일을 비교할 때 각 float element의 차이가 0.001 미만일 때는 동일한 것으로 판정하여 0을 returns하고 그렇지 않으면 1을 return한다.
   compile: gcc -g -Wall -o hwdiffd hwdiffd.c wrapper.c -lm
   실행예: ./hwdiffd c1.dat c2.dat 0.001   
 */
/* 1을 return하는 오류 상황:
   - 첫 번째 파일을 열 수 없거나 n*n double 크기가 아닐 때
...
int main(int argc, char *argv[])
{
  ...
  if (!issquare(afsize)){
    fprintf(stderr, "The 1st file(%s)'s size is not n*n double.\n", argv[1]);
    exit(1);
  }
